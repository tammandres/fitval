...
data_path = PROJECT_ROOT / 'tests' / 'test_data'
path_pred = data_path / 'pred.csv'
path_xy = data_path / 'xy.csv'
path_xy_mis = data_path / 'xy_mis.csv'


# Run with some of the different settings
def test_boot_metrics_when_data_has_predictions():
    data_ci, data_noci = boot_metrics(data_path=path_pred, save_path=test_path, model_names=['logistic-full', 'logistic-fit-age'], 
                                      data_has_predictions=True, B=5, plot_boot=False)


def test_boot_metrics():
    data_ci, data_noci = boot_metrics(data_path=path_xy, save_path=test_path, model_names=['logistic-full', 'logistic-fit-age'], 
                                      data_has_predictions=False, B=5, plot_boot=False)


def test_boot_metrics_mis():
    data_ci, data_noci = boot_metrics(data_path=path_xy_mis, save_path=test_path, model_names=['logistic-full', 'logistic-fit-age'], 
