...
from fitval.boot import boot_metrics, plot_boot_metrics
...
model_names = ['nottingham-cox',
               'nottingham-cox-quant',
               'fit'
               ]
...
save_path = PROJECT_ROOT / 'results' / 'test'
save_path.mkdir(exist_ok=True, parents=True)
print(save_path)


data_ci = boot_metrics(x, y, model_names=model_names, thr_risk=thr_risk, thr_fit=thr_fit,
                       sens=sens, B=B, M=M, save_path=save_path, save=True, parallel=True,
                       return_boot_samples=True)


plot_boot_metrics(data_ci, save_path)
